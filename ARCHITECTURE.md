...
Record entry changes are also reversible ordinary maintenance. The
`ChangeRecord` operation Mutates the stored record at the same identifier,
validates the replacement entry through the same topic discipline as
`Record`, and preserves the daemon-stamped provenance. It exists for
correcting an existing intent record without remove-and-recreate.
...
manager lifecycle surface used by `persona` supervision: `Announce` identifies
the component as `persona-spirit` / `ComponentKind::Spirit`, readiness returns
`Ready`, health returns `Running`, and `Stop` acknowledges the lifecycle
request. Domain Spirit operations never cross this socket.
...
| Record entry changes use the write plane and project to Sema `Mutate`. | `persona_spirit_client_changes_record_entry_without_changing_identifier`, `persona_spirit_record_change_uses_write_plane_and_preserves_identifier`, `spirit_record_change_projects_to_mutated_observation`, and `change_record_replaces_entry_under_same_identifier_and_preserves_provenance` check `ChangeRecord`, `RecordMutationApplied`, replacement entry visibility, identifier preservation, provenance preservation, `RecordMutated`, and `SemaOperation::Mutate`. |
...
| Record subscriptions snapshot record descriptions through the read plane before opening a stream, with the same public-default privacy discipline as record observations. | `persona_spirit_record_subscription_uses_read_plane_then_subscription_plane` checks `SemaReader` before `SubscriptionPlane`; `persona_spirit_client_hides_private_records_from_public_subscription_snapshot` and `persona_spirit_client_can_open_private_record_subscription_snapshot` prove public and explicit private snapshot behavior. |
